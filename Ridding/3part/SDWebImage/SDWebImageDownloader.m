...
- (id)init {
  ...
  self = [super init];
  if (self) {
    _oQueue = [[NSOperationQueue alloc] init];
    _oQueue.maxConcurrentOperationCount = 1;
  }
  return self;
}


- (void)getFirstFrameOperation {
  ...
  NSError *error = nil;
  CGImageRef cgImg = [generator copyCGImageAtTime:CMTimeMake(0, 10) actualTime:NULL error:&error];
...
- (void)start {
  ...
  // 如果是取video的第一帧
  SDWebImageOptions option = [[self.userInfo objectForKey:@"options"] intValue];
  if (option & SDWebImageVideoFirstFrame) {
    NSInvocationOperation *operation = SDWIReturnAutoreleased([[NSInvocationOperation alloc] initWithTarget:self
                                                                                                   selector:@selector(getFirstFrameOperation)
                                                                                                     object:nil]);
    [_oQueue addOperation:operation];
