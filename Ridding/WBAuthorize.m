...
- (void)requestAccessTokenWithAuthorizeCode:(NSString *)code {
  ...
  self.request = [WBRequest requestWithURL:kWBAccessTokenURL httpMethod:@"POST"
                                    params:params
                              postDataType:kWBRequestPostDataTypeNormal
                          httpHeaderFields:nil delegate:self];
...
- (void)startAuthorize {
  ...
  NSDictionary *params = [NSDictionary dictionaryWithObjectsAndKeys:appKey, @"client_id",
                                                                    ...
                                                                    redirectURI, @"redirect_uri",
                                                                    @"mobile", @"display", nil];
  NSString *urlString = [WBRequest serializeURL:kWBAuthorizeURL params:params
                                     httpMethod:@"GET"];
...
- (void)request:(WBRequest *)theRequest didFinishLoadingWithResult:(id)result {
  ...
  if (!success && [delegate respondsToSelector:@selector(authorize:didFailWithError:)]) {
    NSError *error = [NSError errorWithDomain:kWBSDKErrorDomain code:kWBErrorCodeSDK
