...
- (UIImage*)getPhoto:(NSString*)name{
  return [UIImage imageWithData:[NSData dataWithContentsOfFile:[self getPhotoPath:name]]];
}

- (NSString*)getPhotoPath:(NSString*)name{
  return[[self photosPath] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.jpg",name]];
}
...
//                          withObject:nil
//                       waitUntilDone:YES];
  [self releasePhotoList];
  [self sendDidFinishSaveNotification];
...
- (void)deleteImageAtIndex:(NSInteger)index {
  ...
  [photoCache_ removeAllObjects];
  [thumbnailCache_ removeAllObjects];
  if([delegate_ respondsToSelector:@selector(deleteImageAtName:)]){
    [delegate_ deleteImageAtName:name];
  }
